...
class Evaluation():
    ...
    def getROCAUC(self):
        return [roc_auc_score(l, p) for l, p in  zip(self.labels, self.predict)]
    ...
    def getSummary(self, index):
        metrics = {'Accuracy': self.getAccuracy(),'Misclassification': self.getMisclassification(),'Precision': self.getPrecision(),
            'Recall': self.getRecall(), 'F-Score': self.getFScore(), 'TP': self.getTP(), 'FP': self.getFP(),
                   'TN': self.getTN(), 'FN':self.getFN(), 'roc_auc':self.getROCAUC()}
